...
# CA cert/key are managed internally under $DATAX_HOME/proxy/ (ca.pem + ca.key).
# When MITM is active, spawned commands receive CA bundle env vars pointing at
# immutable bundles under $DATAX_HOME/proxy/ so common HTTPS clients trust the managed CA.
