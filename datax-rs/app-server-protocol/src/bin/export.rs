...
#[derive(Parser, Debug)]
#[command(
    about = "Generate TypeScript bindings and JSON Schemas for the Datax app-server protocol"
)]
struct Args {
...
fn main() -> Result<()> {
    ...
    datax_app_server_protocol::generate_ts_with_options(
        ...
        datax_app_server_protocol::GenerateTsOptions {
            experimental_api: args.experimental,
            ..datax_app_server_protocol::GenerateTsOptions::default()
        },
    )?;
    datax_app_server_protocol::generate_json_with_experimental(&args.out_dir, args.experimental)
}
