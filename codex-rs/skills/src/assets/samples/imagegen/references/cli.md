...
Set a stable path to the skill CLI (default `DATAX_HOME` is `~/.datax`):

```
export DATAX_HOME="${DATAX_HOME:-$HOME/.datax}"
export IMAGE_GEN="$DATAX_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$DATAX_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
