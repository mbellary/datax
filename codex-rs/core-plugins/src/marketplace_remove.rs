...
use datax_config::RemoveMarketplaceConfigOutcome;
use datax_config::remove_user_marketplace_config;
...
mod tests {
    ...
    fn remove_marketplace_sync_removes_config_and_installed_root() {
        ...
        let config =
            fs::read_to_string(codex_home.path().join(datax_config::CONFIG_TOML_FILE)).unwrap();
