# Datax package builder

This package contains the implementation behind `scripts/build_datax_package.py`.
...
The builder creates a canonical Datax package directory:
...
│   ├── datax-command-runner.exe          # Windows only
│   └── datax-windows-sandbox-setup.exe   # Windows only
└── datax-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match release
...
- Windows targets: `datax-command-runner` and `datax-windows-sandbox-setup`,
...
`--datax-command-runner-bin` plus `--datax-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Datax-built V8 release pair before invoking
...
DotSlash manifest at `scripts/datax_package/rg`. Downloaded archives are cached
under `$TMPDIR/datax-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/datax_package/datax-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/datax-package/<target>-zsh` and installed at
`datax-resources/zsh/bin/zsh`.
