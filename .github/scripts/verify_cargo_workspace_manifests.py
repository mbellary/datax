...
"""Verify that datax-rs Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the datax-rs directory naming conventions.
...
CARGO_RS_ROOT = ROOT / "datax-rs"
WORKSPACE_PACKAGE_FIELDS = ("version", "edition", "license")
TOP_LEVEL_NAME_EXCEPTIONS = {
    "windows-sandbox-rs": "datax-windows-sandbox",
}
UTILITY_NAME_EXCEPTIONS = {
    "path-utils": "datax-utils-path",
}
MANIFEST_FEATURE_EXCEPTIONS = {
    "datax-rs/code-mode/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
    "datax-rs/v8-poc/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
}
...
def main() -> int:
    ...
    print(
        "Cargo manifests under datax-rs must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `datax-rs/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `datax-rs/<crate>/Cargo.toml` and "
        "`datax-rs/utils/<crate>/Cargo.toml`."
    )
    print(
...
def expected_package_name(path: Path) -> str | None:
    ...
    if len(parts) == 2 and parts[1] == "Cargo.toml":
        ...
        return TOP_LEVEL_NAME_EXCEPTIONS.get(
            directory,
            directory if directory.startswith("datax-") else f"datax-{directory}",
        )
    if len(parts) == 3 and parts[0] == "utils" and parts[2] == "Cargo.toml":
        directory = parts[1]
        return UTILITY_NAME_EXCEPTIONS.get(directory, f"datax-utils-{directory}")
    return None
