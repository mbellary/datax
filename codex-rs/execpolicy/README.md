...
- The legacy rule matcher lives in `datax-execpolicy-legacy`.
...
- From the Datax CLI, run `datax execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
datax execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
datax execpolicy check \
...
```bash
cargo run -p datax-execpolicy -- check --rules path/to/policy.rules git status
```
