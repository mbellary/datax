...
mod tests {
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd datax-rs && rg --files"),
            vec![ParsedCommand::ListFiles {
                cmd: "rg --files".to_string(),
                path: None,
            }],
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        let inner = r#"cd /Users/pakrym/code/codex && rg -n "codex_api" datax-rs -S | head -n 50"#;
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n codex_api datax-rs -S".to_string(),
                query: Some("codex_api".to_string()),
                path: Some("datax-rs".to_string()),
            }],
        );
    }
