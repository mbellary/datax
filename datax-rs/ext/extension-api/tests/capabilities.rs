...
use datax_extension_api::NoopResponseItemInjector;
use datax_extension_api::ResponseItemInjector;
...
async fn closure_agent_spawner_forwards_arguments_and_result() {
    ...
    let spawner = move |chat_id: ChatId,
                        request: String|
    ...
    let chat_id =
        ChatId::from_string("11111111-1111-4111-8111-111111111111").expect("valid thread id");

    let spawned = spawner
        .spawn_subagent(chat_id, "delegate this".to_string())
        .await;

    assert_eq!(spawned, Ok(13));
    assert_eq!(
        calls.lock().expect("agent spawn calls lock").as_slice(),
        [(chat_id, "delegate this".to_string())]
    );
}
