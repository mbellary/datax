...
impl App {
    pub(super) async fn shutdown_current_thread(&mut self, app_server: &mut AppServerSession) {
        if let Some(chat_id) = self.chat_widget.chat_id() {
            ...
            if let Err(err) = app_server.thread_unsubscribe(chat_id).await {
                tracing::warn!("failed to unsubscribe thread {chat_id}: {err}");
            }
            self.abort_thread_event_listener(chat_id);
        }
    }

    pub(super) fn abort_thread_event_listener(&mut self, chat_id: ChatId) {
        if let Some(handle) = self.thread_event_listener_tasks.remove(&chat_id) {
            handle.abort();
        }
    }
    ...
    pub(super) fn ensure_thread_channel(&mut self, chat_id: ChatId) -> &mut ThreadEventChannel {
        self.thread_event_channels
            .entry(chat_id)
            .or_insert_with(|| ThreadEventChannel::new(THREAD_EVENT_CHANNEL_CAPACITY))
    }

    pub(super) async fn set_thread_active(&mut self, chat_id: ChatId, active: bool) {
        if let Some(channel) = self.thread_event_channels.get_mut(&chat_id) {
            let mut store = channel.store.lock().await;
            store.active = active;
        }
    }

    pub(super) async fn activate_thread_channel(&mut self, chat_id: ChatId) {
        if self.active_chat_id.is_some() {
            return;
        }
        self.set_thread_active(chat_id, /*active*/ true).await;
        let receiver = if let Some(channel) = self.thread_event_channels.get_mut(&chat_id) {
    ...
    pub(super) async fn store_active_thread_receiver(&mut self) {
        let Some(active_id) = self.active_chat_id else {
            return;
        };
    ...
    pub(super) async fn activate_thread_for_replay(
        ...
        let channel = self.thread_event_channels.get_mut(&chat_id)?;
    ...
    pub(super) async fn clear_active_thread(&mut self) {
        if let Some(active_id) = self.active_chat_id.take() {
    ...
    pub(super) async fn note_thread_outbound_op(&mut self, chat_id: ChatId, op: &AppCommand) {
        let Some(channel) = self.thread_event_channels.get(&chat_id) else {
            return;
        };
        let mut store = channel.store.lock().await;
    ...
    pub(super) async fn note_active_thread_outbound_op(&mut self, op: &AppCommand) {
        ...
        self.note_thread_outbound_op(chat_id, op).await;
    }

    pub(super) async fn active_interaction_id_for_thread(&self, chat_id: ChatId) -> Option<String> {
        let channel = self.thread_event_channels.get(&chat_id)?;
        let store = channel.store.lock().await;
        store.active_interaction_id().map(ToOwned::to_owned)
    }

    pub(super) fn thread_label(&self, chat_id: ChatId) -> String {
        let is_primary = self.primary_chat_id == Some(chat_id);
        ...
        if let Some(entry) = self.agent_navigation.get(&chat_id) {
    ...
    ///
    /// `active_chat_id` is the source of truth during steady state, but the widget can briefly
    ...
    pub(super) fn current_displayed_chat_id(&self) -> Option<ChatId> {
    ...
    pub(super) fn ignore_same_thread_resume(
        ...
        if self.active_chat_id != Some(target_session.chat_id) {
            return false;
        };
    ...
    pub(super) fn sync_active_agent_label(&mut self) {
        let label = self
            .agent_navigation
            .active_agent_label(self.current_displayed_chat_id(), self.primary_chat_id);
        self.chat_widget.set_active_agent_label(label);
        self.sync_side_thread_ui();
    }

    pub(super) async fn thread_cwd(&self, chat_id: ChatId) -> Option<AbsolutePathBuf> {
        let channel = self.thread_event_channels.get(&chat_id)?;
        let store = channel.store.lock().await;
    ...
    async fn thread_file_change_changes(
        ...
        let channel = self.thread_event_channels.get(&chat_id)?;
        let store = channel.store.lock().await;
        store.file_change_changes(interaction_id, item_id)
    }
    ...
    pub(super) async fn interactive_request_for_thread_request(
        ...
        let thread_label = Some(self.thread_label(chat_id));
        Ok(match request {
    ...
    pub(super) async fn pending_inactive_thread_requests(&self) -> Vec<(ChatId, ServerRequest)> {
        let channels: Vec<(ChatId, Arc<Mutex<ThreadEventStore>>)> = self
            .thread_event_channels
            .iter()
            .map(|(chat_id, channel)| (*chat_id, Arc::clone(&channel.store)))
            .collect();

        let mut requests = Vec::new();
        for (chat_id, store) in channels {
            if Some(chat_id) == self.active_chat_id {
                continue;
            }
    ...
    pub(super) async fn surface_pending_inactive_thread_interactive_requests(
        ...
        if self.active_side_parent_chat_id().is_some() {
            return Ok(());
        }

        let requests = self.pending_inactive_thread_requests().await;
        for (chat_id, request) in requests {
    ...
    pub(super) async fn submit_thread_op(
        ...
        if self
            .try_resolve_app_server_request(app_server, chat_id, &op)
            .await?
        {
            return Ok(());
        }
        ...
        if self
            .try_submit_active_thread_op_via_app_server(app_server, chat_id, &op)
            .await?
        {
            if ThreadEventStore::op_can_change_pending_replay_state(&op) {
                self.note_thread_outbound_op(chat_id, &op).await;
                self.refresh_pending_thread_approvals().await;
                self.refresh_side_parent_status_from_store(chat_id).await;
            }
            return Ok(());
        }

        self.chat_widget
            .add_error_message(format!("Not available in TUI yet for thread {chat_id}."));
        Ok(())
    }

    /// Persist prompt text in the local cross-session message history.
    pub(super) fn append_message_history_entry(&self, chat_id: ChatId, text: String) {
        ...
        tokio::spawn(async move {
            if let Err(err) =
                datax_message_history::append_entry(&text, chat_id, &history_config).await
            {
                tracing::warn!(
                    chat_id = %chat_id,
                    error = %err,
    ...
    pub(super) async fn try_submit_active_thread_op_via_app_server(
        ...
        match op {
            AppCommand::Interrupt { .. } => {
                if let Some(interaction_id) = self.active_interaction_id_for_thread(chat_id).await {
                    let mut interrupt_interaction_id = interaction_id;
                    for retried_after_turn_mismatch in [false, true] {
                        match app_server
                            .turn_interrupt(chat_id, interrupt_interaction_id.clone())
                            .await
                        {
                            Ok(()) => return Ok(true),
                            Err(error) if !retried_after_turn_mismatch => {
                                let Some(actual_interaction_id) = active_turn_interrupt_race(&error)
                                else {
                                    return Err(error).wrap_err("turn/interrupt failed in TUI");
                                };
                                if actual_interaction_id == interrupt_interaction_id {
                                    return Err(error).wrap_err("turn/interrupt failed in TUI");
                                }
                                ...
                                // active turn id.
                                interrupt_interaction_id = actual_interaction_id;
                            }
                            Err(error) => {
                                return Err(error).wrap_err("turn/interrupt failed in TUI");
                            }
            ...
            AppCommand::UserTurn {
                ...
                if let Some(interaction_id) = self.active_interaction_id_for_thread(chat_id).await {
                    let mut steer_interaction_id = interaction_id;
                    let mut retried_after_turn_mismatch = false;
                    loop {
                        match app_server
                            .turn_steer(chat_id, steer_interaction_id.clone(), items.to_vec())
                            .await
                        {
                            Ok(_) => return Ok(true),
                            Err(error) => {
                                ...
                                match active_turn_steer_race(&error) {
                                    Some(ActiveTurnSteerRace::Missing) => {
                                        if let Some(channel) =
                                            self.thread_event_channels.get(&chat_id)
                                        {
                                            let mut store = channel.store.lock().await;
                                    ...
                                    Some(ActiveTurnSteerRace::ExpectedTurnMismatch {
                                        ...
                                        && actual_interaction_id != steer_interaction_id =>
                                    {
                                        ...
                                        {
                                            let mut store = channel.store.lock().await;
                                            store.active_interaction_id = Some(actual_interaction_id.clone());
                                        }
                                        steer_interaction_id = actual_interaction_id;
                                    ...
                                    Some(ActiveTurnSteerRace::ExpectedTurnMismatch {
                                        ...
                                        {
                                            let mut store = channel.store.lock().await;
                                            store.active_interaction_id = Some(actual_interaction_id);
                                        }
            ...
            AppCommand::Compact => {
                app_server.thread_compact_start(chat_id).await?;
                Ok(true)
            }
            AppCommand::SetThreadName { name } => {
                app_server
                    .thread_set_name(chat_id, name.to_string())
                    .await?;
                Ok(true)
            }
            AppCommand::ThreadRollback { num_turns } => {
                let response = match app_server.thread_rollback(chat_id, *num_turns).await {
                ...
                self.handle_thread_rollback_response(chat_id, *num_turns, &response)
                    .await;
                Ok(true)
            }
            AppCommand::Review { target } => {
                let response = app_server.review_start(chat_id, target.clone()).await?;
                let review_chat_id = ChatId::from_string(&response.review_chat_id)
            ...
            AppCommand::CleanBackgroundTerminals => {
                app_server
                    .thread_background_terminals_clean(chat_id)
                    .await?;
                Ok(true)
            }
            AppCommand::RunUserShellCommand { command } => {
                app_server
                    .thread_shell_command(chat_id, command.to_string())
                    .await?;
                Ok(true)
            }
            ...
            AppCommand::ApproveGuardianDeniedAction { event } => {
                app_server
                    .thread_approve_guardian_denied_action(chat_id, event)
                    .await?;
                Ok(true)
            }
            _ => Ok(false),
            ...
            .await
        {
            ...
            Err(err) => {
                self.chat_widget.add_error_message(format!(
                    "Failed to resolve app-server request for thread {chat_id}: {err}"
                ));
                Ok(false)
            }
        }
    ...
    pub(super) async fn refresh_pending_thread_approvals(&mut self) {
        let side_parent_chat_id = self.active_side_parent_chat_id();
        let channels: Vec<(ChatId, Arc<Mutex<ThreadEventStore>>)> = self
            .thread_event_channels
            .iter()
            .map(|(chat_id, channel)| (*chat_id, Arc::clone(&channel.store)))
        ...
        let mut pending_chat_ids = Vec::new();
        for (chat_id, store) in channels {
            if Some(chat_id) == self.active_chat_id || Some(chat_id) == side_parent_chat_id
            {
                continue;
            }
            ...
            if store.has_pending_thread_approvals() {
                pending_chat_ids.push(chat_id);
            }
        }

        pending_chat_ids.sort_by_key(ChatId::to_string);

        let threads = pending_chat_ids
            .into_iter()
            .map(|chat_id| self.thread_label(chat_id))
            .collect();

        self.chat_widget.set_pending_thread_approvals(threads);
    }

    pub(super) async fn refresh_side_parent_status_from_store(&mut self, chat_id: ChatId) {
        let Some(channel) = self.thread_event_channels.get(&chat_id) else {
            return;
        };
        let status = {
        ...
        if let Some(status) = status {
            self.set_side_parent_status(chat_id, Some(status));
        } else {
            self.clear_side_parent_action_status(chat_id);
        }
    }
    ...
    pub(super) async fn enqueue_thread_notification(
        ...
        if matches!(notification, ServerNotification::ChatSettingsUpdated(_))
            && self.primary_chat_id.is_some()
            && self.primary_chat_id != Some(chat_id)
            && !self.thread_event_channels.contains_key(&chat_id)
        {
            return Ok(());
        }
        if let ServerNotification::ChatSettingsUpdated(notification) = &notification {
            self.apply_thread_settings_to_cached_session(chat_id, &notification.thread_settings)
                .await;
        }
        let inferred_session = self
            .infer_session_for_thread_notification(chat_id, &notification)
            .await;
        let (sender, store) = {
        ...
        if let Some(status) = pending_status {
            ...
            self.apply_side_parent_status_change(chat_id, change);
        }
        self.refresh_pending_thread_approvals().await;
        Ok(())
    }
    ...
    pub(super) fn cache_collab_receiver_threads_for_notification(
        ...
        let Some(receiver_chat_ids) = collab_receiver_chat_ids(notification) else {
            return;
        };

        for receiver_chat_id in receiver_chat_ids {
            if collab_receiver_is_not_found(notification, receiver_chat_id) {
                continue;
            }

            let Ok(chat_id) = ChatId::from_string(receiver_chat_id) else {
                tracing::warn!(
            ...
            if self.agent_navigation.get(&chat_id).is_some() {
                continue;
            }
    ...
    pub(super) async fn enqueue_thread_request(
        ...
        let inactive_interactive_request = if self.active_chat_id != Some(chat_id) {
            self.interactive_request_for_thread_request(chat_id, &request)
                .await?
        } else {
            None
        };
        ...
        } else if self.active_side_parent_chat_id().is_none()
    ...
    pub(super) async fn enqueue_primary_thread_session(
        ...
        self.activate_thread_channel(chat_id).await;
        self.chat_widget
    ...
    pub(super) async fn enqueue_primary_thread_notification(
        ...
        if let Some(chat_id) = self.primary_chat_id {
            return self
                .enqueue_thread_notification(chat_id, notification)
                .await;
        }
    ...
    pub(super) async fn enqueue_primary_thread_request(
        ...
        if let Some(chat_id) = self.primary_chat_id {
            return self.enqueue_thread_request(chat_id, request).await;
        }
        self.pending_primary_events
    ...
    pub(super) async fn refresh_snapshot_session_if_needed(
        ...
        if !self.should_refresh_snapshot_session(chat_id, is_replay_only, snapshot) {
            return;
        }

        match app_server
            .resume_thread(self.config.clone(), chat_id)
            .await
        {
            Ok(started) => {
                self.apply_refreshed_snapshot_thread(chat_id, started, snapshot)
                    .await
            }
            Err(err) => {
                tracing::warn!(
    ...
    pub(super) fn should_refresh_snapshot_session(
        ...
        !is_replay_only
            && !self.side_threads.contains_key(&chat_id)
    ...
    /// Returns `(closed_chat_id, primary_chat_id)` when a non-primary active
    /// thread has died and we should fail over to the primary thread.
    ///
    /// A user-requested shutdown (`ExitMode::ShutdownFirst`) sets
    /// `pending_shutdown_exit_chat_id`; matching shutdown completions are ignored
    ...
    pub(super) fn active_non_primary_shutdown_target(
        ...
        if self.pending_shutdown_exit_chat_id == Some(active_chat_id) {
            return None;
        }
        (active_chat_id != primary_chat_id).then_some((active_chat_id, primary_chat_id))
    }
    ...
    pub(super) fn replay_thread_snapshot(
        ...
        if let Some(session) = snapshot.session {
            if self.side_threads.contains_key(&session.chat_id) {
    ...
    pub(super) fn should_stop_waiting_for_initial_session(
        ...
        waiting_for_initial_session_configured && primary_chat_id.is_some()
    }
    ...
    pub(super) async fn handle_active_thread_event(
        ...
        if let ThreadBufferedEvent::Notification(notification) = &event
            && let Some((closed_chat_id, primary_chat_id)) =
                self.active_non_primary_shutdown_target(notification)
        {
            self.mark_agent_picker_thread_closed(closed_chat_id);
            if self.side_threads.contains_key(&closed_chat_id) {
                self.discard_closed_side_thread(closed_chat_id).await;
                self.select_agent_thread(tui, app_server, primary_chat_id)
                    .await?;
            } else {
                self.select_agent_thread_and_discard_side(tui, app_server, primary_chat_id)
                    .await?;
            }
            if self.active_chat_id == Some(primary_chat_id) {
                self.chat_widget.add_info_message(
                    format!(
                        "Agent thread {closed_chat_id} closed. Switched back to main thread."
                    ),
                    /*hint*/ None,
                );
            } else {
                self.clear_active_thread().await;
                self.chat_widget.add_error_message(format!(
                    "Agent thread {closed_chat_id} closed. Failed to switch back to main thread {primary_chat_id}.",
                ));
            }
            return Ok(());
        }
