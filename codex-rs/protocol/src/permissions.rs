...
const PROTECTED_METADATA_DATAX_PATH_NAME: &str = ".datax";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_DATAX_PATH_NAME,
];
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        || name == OsStr::new(PROTECTED_METADATA_DATAX_PATH_NAME)
}
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".datax");
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_datax*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_datax*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_datax*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_dot_datax = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_datax)
                    .into_iter()
            ...
            // Example: if `<root>/.datax -> <root>/decoy`, bwrap must still see
            // `<root>/.datax`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.datax` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_datax: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .datax read-only to the agent by
    ...
    let top_level_datax = writable_root.join(PROTECTED_METADATA_DATAX_PATH_NAME);
    if protect_missing_dot_datax || top_level_datax.as_path().is_dir() {
        subpaths.push(top_level_datax);
    }
...
mod tests {
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".datax".into())),
                    },
                    access: FileSystemAccessMode::Read,
                },
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_datax_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_datax = expected_root.join(".datax");
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".datax".to_string()),
            "explicit .datax rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_datax),
            "explicit .datax rule should win over the default protected carveout"
        );
        assert!(
            policy.can_write_path_with_cwd(
                explicit_dot_datax.join("config.toml").as_path(),
                cwd.path()
            )
        );
    }
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_datax_config = cwd.path().join(".datax").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_datax_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_datax_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_datax*/ true,
            )
            .into_iter()
            .map(|path| FileSystemSandboxEntry {
                path: FileSystemPath::Path { path },
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".datax/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let datax_dir = real_root.join(".datax");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&datax_dir).expect("create .datax");
        ...
        let expected_datax = link_root.join(".datax");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_datax = expected_root.join(".datax");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-datax");
        let dot_datax = root.join(".datax");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_datax).expect("create .datax symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_datax = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".datax"),
        )
        .expect("absolute .datax symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let datax_dir = real_tmpdir.join(".datax");
