...
//!
//! If the keyring is not available or fails, we fall back to DATAX_HOME/.credentials.json which is consistent with other coding CLI agents.
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create DATAX_HOME temp dir");
            unsafe {
                std::env::set_var("DATAX_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
