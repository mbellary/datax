...
`datax-rs/core/src/memories/`.

## Crates

- `datax-rs/memories/read` (`codex-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `datax-rs/memories/write` (`codex-memories-write`) owns the write path:
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.datax/memories/.git` by `codex-git-utils`
