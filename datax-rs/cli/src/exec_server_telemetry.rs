...
const OTEL_SERVICE_NAME: &str = "datax-exec-server";

pub(crate) fn init(
    config: Option<&datax_core::config::Config>,
) -> Result<impl Send + Sync, Box<dyn std::error::Error>> {
    let fmt_layer = tracing_subscriber::fmt::layer()
        .with_writer(std::io::stderr)
        .with_filter(stderr_env_filter());
    let otel = match config {
        Some(config) => datax_core::otel_init::build_provider(
            config,
    ...
    datax_core::otel_init::record_process_start(provider, OTEL_SERVICE_NAME);
