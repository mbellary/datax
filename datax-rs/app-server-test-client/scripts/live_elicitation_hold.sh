...
chat_id="${CODEX_THREAD_ID:-${THREAD_ID-}}"
if [ -z "$chat_id" ]; then
...
cleanup() {
  if [ "$incremented" -eq 1 ]; then
    "$APP_SERVER_TEST_CLIENT_BIN" --url "$APP_SERVER_URL" \
      thread-decrement-elicitation "$chat_id" >/dev/null 2>&1 || true
  fi
}

trap cleanup EXIT INT TERM HUP

echo "[elicitation-hold] increment thread=$chat_id"
"$APP_SERVER_TEST_CLIENT_BIN" --url "$APP_SERVER_URL" \
  thread-increment-elicitation "$chat_id"
incremented=1

echo "[elicitation-hold] sleeping ${hold_seconds}s"
sleep "$hold_seconds"

echo "[elicitation-hold] decrement thread=$chat_id"
"$APP_SERVER_TEST_CLIENT_BIN" --url "$APP_SERVER_URL" \
  thread-decrement-elicitation "$chat_id"
incremented=0
