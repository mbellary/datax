...
```shell
cargo run -p datax-execpolicy-legacy -- check ls -l foo | jq
```
...
```shell
cargo run -p datax-execpolicy-legacy -- check cp src1 src2 dest | jq
```
...
```shell
cargo run -p datax-execpolicy-legacy -- check applied deploy | jq
```
