...
This repository uses Bazel to build the Rust workspace under `datax-rs`.
...
- `rules_rs` imports third-party crates from `datax-rs/Cargo.toml` and
  `datax-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
...
- Each crate in `datax-rs/*/BUILD.bazel` typically uses `codex_rust_crate` and
