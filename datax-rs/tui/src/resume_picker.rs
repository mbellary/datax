...
impl SessionTarget {
    pub fn display_label(&self) -> String {
        self.path
            ...
            .unwrap_or_else(|| format!("thread {}", self.chat_id))
    }
}
    ...
    tokio::spawn(async move {
        ...
        while let Some(request) = request_rx.recv().await {
            match request {
                ...
                PickerLoadRequest::Preview { chat_id } => {
                    let preview = load_transcript_preview(&mut app_server, chat_id).await;
...
impl Row {
    fn seen_key(&self) -> Option<SeenRowKey> {
        ...
        self.chat_id.map(SeenRowKey::Chat)
    }
...
mod tests {
    ...
    fn footer_branch_expands_when_line_has_room() {
        ...
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some(branch),
            Some("~/code/codex.etraut-animations-false-improvements/datax-rs"),
            /*show_cwd*/ true,
            /*width*/ 140,
        );
    ...
    fn footer_cwd_truncates_to_responsive_column() {
        let cwd = "~/code/codex.owner-extremely-long-worktree-name-that-needs-truncating/datax-rs";
    ...
    fn footer_omits_cwd_when_hidden() {
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some("owner/branch"),
            Some("~/code/codex.owner-worktree/datax-rs"),
            /*show_cwd*/ false,
            /*width*/ 80,
        );
    ...
    fn dense_snapshot_row() -> Row {
        Row {
            ...
            cwd: Some(PathBuf::from(
                "/Users/felipe.coury/code/codex.fcoury-session-picker/datax-rs",
            )),
            git_branch: Some(String::from("fcoury/session-picker")),
        }
    }
