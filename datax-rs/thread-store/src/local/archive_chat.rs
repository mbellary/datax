...
pub(super) async fn archive_chat(
    ...
    let canonical_rollout_path = scoped_rollout_path(
        store.config.codex_home.join(datax_rollout::SESSIONS_SUBDIR),
    ...
    if let Some(ctx) = state_db_ctx {
        let _ = ctx
            .mark_archived(chat_id, archived_path.as_path(), Utc::now())
            .await;
    }
    Ok(())
}
...
mod tests {
    ...
    async fn archive_chat_moves_rollout_to_archived_collection() {
        ...
        assert_eq!(archived.items[0].chat_id, chat_id);
