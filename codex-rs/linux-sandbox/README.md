...
- a `datax-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Datax CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `datax-exec` CLI can check if its arg0 is `datax-linux-sandbox` and, if so, execute as if it were `datax-linux-sandbox`
  - this should also be true of the `datax` multitool CLI

On Linux, Datax prefers the first `bwrap` found on `PATH`
...
with Datax.
Datax also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Datax surfaces the same startup warning
...
- If `bwrap` is missing, Datax also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Datax surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Datax rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.datax`) are re-applied as read-only via `--ro-bind`.
