...
async fn exec_full_buffer_capture_ignores_expiration() -> Result<()> {
    ...
    let output = exec(
        ExecParams {
            command,
            cwd: datax_utils_absolute_path::AbsolutePathBuf::current_dir()?,
            expiration: 1.into(),
...
async fn process_exec_tool_call_preserves_full_buffer_capture_policy() -> Result<()> {
    ...
    let cwd = datax_utils_absolute_path::AbsolutePathBuf::current_dir()?;
...
fn windows_elevated_allows_split_restricted_read_policies() {
    ...
    let docs = datax_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
fn windows_restricted_token_rejects_split_only_filesystem_policies() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        datax_protocol::permissions::FileSystemSandboxEntry {
            path: datax_protocol::permissions::FileSystemPath::Path {
                path: datax_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
                    .expect("absolute docs"),
            },
            access: datax_protocol::permissions::FileSystemAccessMode::Read,
        },
...
fn windows_restricted_token_supports_full_read_split_write_read_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        datax_protocol::permissions::FileSystemSandboxEntry {
            path: datax_protocol::permissions::FileSystemPath::Path { path: docs.clone() },
            access: datax_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);
    let permission_profile = PermissionProfile::from_runtime_permissions(
    ...
    // `.datax`, so the restricted-token overlay only needs the extra read-only
    // docs carveout.
...
fn windows_restricted_token_rejects_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        datax_protocol::permissions::FileSystemSandboxEntry {
            path: datax_protocol::permissions::FileSystemPath::Path { path: blocked },
...
fn windows_elevated_supports_split_write_read_carveouts() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            additional_deny_read_paths: vec![],
            additional_deny_write_paths: vec![
                datax_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_docs)
                    .expect("absolute docs"),
            ],
        }))
    );
...
fn windows_elevated_supports_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        datax_protocol::permissions::FileSystemSandboxEntry {
            path: datax_protocol::permissions::FileSystemPath::Path {
                path: datax_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&blocked)
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                datax_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
                    expected_blocked.clone(),
                )
                .expect("absolute blocked"),
            ],
            additional_deny_write_paths: vec![
                datax_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_blocked)
                    .expect("absolute blocked"),
            ],
        }))
    );
...
fn windows_elevated_supports_unreadable_globs() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                datax_utils_absolute_path::AbsolutePathBuf::from_absolute_path(secret)
...
fn windows_elevated_rejects_reopened_writable_descendants() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        datax_protocol::permissions::FileSystemSandboxEntry {
            path: datax_protocol::permissions::FileSystemPath::Path {
                path: datax_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&nested)
...
fn process_exec_tool_call_uses_platform_sandbox_for_network_only_restrictions() {
    let expected = datax_sandboxing::get_platform_sandbox(/*windows_sandbox_enabled*/ false)
